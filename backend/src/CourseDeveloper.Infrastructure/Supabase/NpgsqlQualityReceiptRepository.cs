...
public class NpgsqlQualityReceiptRepository
{
    private readonly NpgsqlDataSource _dataSource;
    private static readonly JsonSerializerOptions _jsonOptions = new() { PropertyNamingPolicy = JsonNamingPolicy.CamelCase };

    public NpgsqlQualityReceiptRepository(NpgsqlDataSource dataSource)
    {
        ...
        try
        {
            using var cmd = new NpgsqlCommand(@"
                INSERT INTO quality_receipts (
                    ...
                    @id, @project_id, @session_id, @stage_name::text, @overall_verdict::text, @detailed_receipt::jsonb, @created_at
                )", conn, tx);
            ...
            foreach (var result in receipt.GateResults)
            {
                ...
                using var resCmd = new NpgsqlCommand(@"
                    INSERT INTO quality_gate_results (
                        ...
                        @id, @receipt_id, @gate_code, @verdict::text, @metric_value, @detail, @evidence::jsonb, @created_at
                    )", conn, tx);
