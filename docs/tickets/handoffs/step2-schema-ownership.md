...
**Resolved 2026-09-04 (DEC-006):** the user approved Studio's project (`gjxhfyfonjdcaimxjipp`) as canonical. MVP's tables get created there via Prisma, MVP's production data is copied across in a one-time migration, and MVP's old project (`jbjfafyjqdjmzmdggzha`) is retired once cutover is verified.

Consolidation is still a real data migration (existing teachers, worksheets, approvals) with an actual cutover, not merely a config change. DEC-006 assigns that sequencing to STEP 8; this handoff does not claim the transfer or cutover has happened. STEP 8 must also confirm the deployed environment's actual `SUPABASE_CONNECTION_STRING`/refs match Studio's checked-in defaults before treating this as settled — no Studio `.env` is present in this repo to verify against directly.
