---
title: "Educational Decisions & Curricular Trace — Lec 11"
session_code: "Lec 11"
...
# 🏛️ Curricular Decisions & Pedagogical Design Record: Lec 11
...
**Topic**: Gas Chromatography (GC): Columns, Stationary Phases, Detectors (FID, TCD, ECD)  
**Contact Hours**: 1 Hour Theory, 2 Hours Practical Laboratory (Practical Exam 2 (Final Practical Assessment))  
...
- **Curricular Module**: Chromatography & Separation Sciences (represents 50% of the theoretical curriculum and 37.5 marks on the accredited final exam).
- **Module Lecturer**: Dr. Shereen Shalan.
...
[Level 3: Apply]       --> Calculate numerical quantities using $$u = \frac{L}{t_M} \quad | \quad I = 100 \left[ n + \frac{\log t_R'(x) - \log t_R'(n)}{\log t_R'(n+1) - \log t_R'(n)} \right] \quad (\text{Kovats Index})$$.
...
2. **Intermediate Cognitive Ascent (Understand & Apply)**:
   - Walk step-by-step through mathematical calculations ($$u = \frac{L}{t_M} \quad | \quad I = 100 \left[ n + \frac{\log t_R'(x) - \log t_R'(n)}{\log t_R'(n+1) - \log t_R'(n)} \right] \quad (\text{Kovats Index})$$) with explicit units ($L \cdot mol^{-1} \cdot cm^{-1}$ or theoretical plates $N$).
...
Scope of GC for volatile, thermally stable compounds, mobile carrier gases (He, N2, H2), sample injection systems (split/splitless heated injector, headspace analysis), columns (packed columns vs fused silica open tubular FSOT capillary columns: WCOT, SCOT, PLOT), stationary liquid phases (polysiloxanes, polyethylene glycols), temperature programming, detectors (Flame Ionization Detector FID, Thermal Conductivity Detector TCD, Electron Capture Detector ECD), derivatization (silylation, alkylation) for non-volatile polar pharmaceuticals.

### B. Strict Non-Forced Application Rationale
> **Pedagogical Governance Rule**:
> Covers vapor-phase thermodynamics, carrier gas pneumatics, capillary column wall coatings, and ionization detectors. Residual solvent analysis per USP <467> serves as compendial quality control benchmark.
> 
...
GC-MS spectral fragmentation library matching algorithms.
...
- All ingested media met pedagogical threshold; zero decorative clip-art identified.
